...
const wfItemList = require("warframe-item-list")
const cmdItem = async info => {
  ...
  const weapon = wfItemList.objects[global.itemType].find(i => i.name == global.itemName);
...
const cmdCetus = async (info, client) => {
  ...
  if (now > activation && now < expiry) {
    ...
    cetus.timeLeft = formatCetus(((cetus.cycle=='Day'?100:150) * 60 * 1000 - timeSinceDusk) / 1000);
  }
...
const cmdBaro = async info => {
  ...
  if (baro.timedate >= baro.expiry) {
    ...
    if(texpiry >= 0) {
      const t = retFormat(baro.expiry - baro.timedate)
  ...
  const t = retFormat(baro.activation - baro.timedate)
